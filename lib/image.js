...
class Scene {
  ...
  draw_full_image(name, w, h) {
    const image = this._[name]
    if (image && image._ && image._._ && this.ctx) {
      this.ctx.drawImage(image._._, image.x, image.y, image.w, image.h, 0, 0, w, h)
    } else {
  ...
  draw_full(name, w, h) {
    const image = this._[name]
    if (image && image._ && image._._ && this.ctx) {
      this.ctx.drawImage(image._._, 0, 0)
    } else {
      console.error("ctx is not set or image is not available")
    }
  }
  ...
  draw(name, x, y, w, h) {
    ...
    if (image && image._ && image._._ && this.ctx) {
      const dw = image.dw || w;
      const dh = image.dh || h;
      this.ctx.drawImage(image._._, image.x, image.y, image.w, image.h, x - image.w/2, y - image.h/2, dw, dh)
    } else {
      console.error("ctx is not set or image is not available")
    }
  }
