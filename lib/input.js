  ...
  touchend(e) {
    ...
    if (e.changedTouches.length > 0) {
      ...
      if (this._on(e.pageX, e.pageY)) {
        ...
        if (this.state[t.id] && this.state[t.id].start) this._.status |= ButtonEvent.press
      }
      delete this.state[t.id]
    }
  }
  ...
  on(e) {
    ...
    for (let i = 0; i < e.touches.length; i++) {
      ...
      if (this._on(t.pageX, t.pageY)) {
        ...
        touched = true
        state.identifier = { x: t.pageX, y: t.pageY, start: false }
      }
    }
